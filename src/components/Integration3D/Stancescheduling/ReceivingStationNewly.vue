<template>
    <div class="TranFe_maximum">
        ...
        <!-- 站位质量信息-弹框 -->
        <div class="black" v-show="PositionCommandVisible"></div>
        <div v-show="PositionCommandVisible" class="renyuan">
            <div>
                <h5>站位质量信息<i class="el-icon-close" @click="iconclose()"></i></h5></h5>
                <div>
        ...
        <!-- 拆解指令信息-弹框 -->
        <div class="black" v-show="DismantlingInstructionsVisible"></div>
        <div v-show="DismantlingInstructionsVisible" class="renyuan">
            <div>
                <h5>拆解指令信息<i class="el-icon-close" @click="iconclose()"></i></h5></h5>
                <div>
                    <el-table
                        border
...
export default {
    ...
    created() {
        ...
        let data = JSON.parse(localStorage.getItem('turnoverlist'))
        this.handoverId = data.handoverId
        this.HanPage = data.HanPage
        this.OnQuery();
    },
    methods: {
        // 关闭弹窗
        iconclose(){
            this.PositionCommandVisible = false
            this.DismantlingInstructionsVisible = false
        },
        //查询接收信息确认信息
        OnQuery() {
            this.axios
