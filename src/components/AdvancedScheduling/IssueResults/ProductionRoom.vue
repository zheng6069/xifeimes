...
export default {
    data() {
        return {
            ...
            optionOne: [],
            feedback: "", // 型号室备注
            workRebutRemark: "", // 生产室备注
            planRebutRemark: "" // 站位负责人驳回备注
        };
    },
    created() {
    ...
    methods: {
        ...
        returnSava() {
            ...
            if (that.tableData.length == 0) {
                ...
                if (
                    that.workRebutRemark == "" ||
                    that.workRebutRemark == undefined
                ) {
                    that.$message({
                        message: "生产室备注为必填项",
                        type: "error"
                    ...
                    let url =
                        AutomaticScheduling.GET_Q_monthDisagree +
                        "?monthStatus=" +
                        1;
                    that.axios
                        .post(url, that.tableData)
                        .then(res => {
        ...
        submitSava() {
            ...
            if (that.tableData.length != 0) {
                let url =
                    AutomaticScheduling.GET_Q_monthAgree +
                    "?message=" +
                    that.workRebutRemark +
                    "&monthStatus=" +
                    3;
