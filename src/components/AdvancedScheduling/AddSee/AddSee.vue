...
export default {
    ...
    methods: {
        gaodu() {
            ...
            this.TableHeight.height = window.innerHeight - 155 + "px";
        },
        showtabs() {
        ...
        findAllMaterialList() {
            let that = this;
            let url = AutomaticScheduling.GET_Z_findMaterialProductList;
            let data = {
                sortieNo: that.sortieNum,
                stationName: that.stanceNum,
                strTime: that.materialTime
            };
            that.axios
                .post(url, data)
                .then(res => {
                    that.tableDataOne = res.data.list;
                })
                .catch(error => {
                    console.log(error);
                });
        ...
        findAllEquList() {
            let that = this;
            let url = AutomaticScheduling.GET_Z_findEqu;
            let data = {
                time: that.equTime
            };
            that.axios
        ...
        findAllToolList() {
            let that = this;
            let url = AutomaticScheduling.GET_Z_findTool;
            let data = {
                time: that.toolTime
            };
            that.axios
        ...
        findAllMeasureList() {
            let that = this;
            let url = AutomaticScheduling.GET_Z_findResultToolsInfo;
            let data = {
                list: that.measureTime
            };
            that.axios
                .post(url, data)
                .then(res => {
                    that.tableDataFour = res.data.list;
                })
                .catch(error => {
                    console.log(error);
                });
