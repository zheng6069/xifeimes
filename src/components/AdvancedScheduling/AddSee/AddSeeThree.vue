<template>
    <div>
        <div class="AddSeeThreehead">
            <div class="AddSeeThreehead_left">
                <span class="resultSpan">结果反馈： {{ result == "1" ? "其他" : "准备就绪" }}</span>
            </div>
            <div class="AddSeeThreehead_left" style="width: 300px">
        ...
        <div class="titles">工装信息</div>
        <div class="AddSeeThreecontnet" :style="AddSeeThreeTableHeight">
            <div class="AddSeeThreecontnet_table">
                <el-table
                    :data="tableData"
                    style="width: 100%"
                    height="100%"
                ...
                >
                    ...
                    <el-table-column label="工装编号" prop="toolingNo"></el-table-column>
                    <el-table-column label="工装图号" prop="drawingNo"></el-table-column>
                    <el-table-column label="工装规格" prop="specifications"></el-table-column>
                </el-table>
            </div>
        </div>
        <div class="AddSeeThreeDetectPaging">
            <el-pagination
                background
...
export default {
    ...
    methods: {
        gaodu() {
            ...
            this.AddSeeThreeTableHeight.height =
                window.innerHeight - 150 + "px";
        },
        // 查询
        getDataList() {
            let that = this;
            let url = AutomaticScheduling.GET_Z_findToolInfo;
            let data = {
                page: that.nowPages,
                limit: that.pageSize,
            ...
            that.axios
                ...
                .then(res => {
                    ...
                    that.result = res.data.records[0].list[0].feedbackResults;
                    let timeArr = [];
                    timeArr.push(res.data.records[0].list[0].confirmStartTime);
                    timeArr.push(res.data.records[0].list[0].confirmEndTime);
                    this.time = timeArr;
                })
                .catch(error => {
                    console.log(error);
...
.AddSeeThreehead_left {
    width: 250px;
    margin-top: 10px;
    margin-left: 10px;
    float: left;
}
.AddSeeThreehead_left p {
    width: 80px;
    float: left;
    line-height: 36px;
}
.AddSeeThreehead_left .resultSpan {
    height: 36px;
    line-height: 36px;
}
.AddSeeThreehead_left div {
    width: 170px;
    float: left;
}
.AddSeeThreehead_bnt {
...
.AddSeeThreeTesting div {
    width: 160px;
    float: right;
}
.AddSeeThreeTaskContent {
    width: 510px;
    height: 80px;
    overflow: hidden;
    float: left;
...
.AddSeeThreecontnet_table {
    padding: 0px 10px;
    height: 100%;
}

.AddSeeThreeDetectPaging {
    height: 50px;
    padding: 0px 10px;
    margin-top: 5px;
}
