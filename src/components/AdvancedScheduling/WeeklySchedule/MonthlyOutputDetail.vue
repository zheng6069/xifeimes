...
export default {
    data() {
        return {
            ...
            weekPlanId: this.$route.query.weekPlanId,
            weekPlanName: this.$route.query.weekPlanName,
            formInline: {
                one: "", // 任务来源
                two: "", // 完成标志
                three: "", // 负责人
    ...
    methods: {
        ...
        seeList() {
            let _self = this;
            if (_self.weekPlanName == "查看") {
                _self.axios
                    .get(AutomaticScheduling.GET_O_findByWeekPlanId, {
                        params: {
                            weekPlanId: _self.weekPlanId
                        }
                    })
                    .then(res => {
                        _self.QueryArr = res.data.tdWeekPlanInstructs;
                ...
                _self.axios
                    ...
                    .then(res => {
                        _self.QueryArr = res.data.tdWeekPlanInstructs;
                        _self.qPush = res.data.tdWeekPlanInstructs;
                        _self.formInline.one = res.data.taskSource;
                        _self.formInline.two = res.data.finishFlag;
                        _self.formInline.three = res.data.principal;
                        _self.formInline.four = res.data.workContent;
        ...
        IRTItemQuery() {
            let that = this;
            that.axios
                .post(AutomaticScheduling.GET_O_findAllWeekPlan, {
        ...
        dialogSave() {
            this.multipleSelection.map(item => {
                ...
                let peon = this.qPush.reduce((cur, next) => {
                    obj[next.planInstructId] ? "" : obj[next.planInstructId] = true && cur.push(next);
                    return cur;
            ...
            this.$refs[formInline].validate(valid => {
                if (valid) {
                    if (that.QueryArr.length != 0) {
                        if (that.weekPlanName == "增加") {
                            // 新增
                            that.axios
                                .post(AutomaticScheduling.GET_O_addWeekPlan, {
                                    tdWeekPlanInstructs: that.QueryArr,
                        ...
                        } else if (that.weekPlanName == "修改") {
                            // 修改
                            that.axios
                                .post(
                                    AutomaticScheduling.GET_O_updateWeekPlan,
                                    {
                                        weekPlanId: that.weekPlanId,
                                        tdWeekPlanInstructs: that.QueryArr,
                                        taskSource: that.formInline.one,
                                        workContent: that.formInline.four,
                                        principal: that.formInline.three
