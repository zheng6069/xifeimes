...
export default {
    ...
    methods: {
        ...
        returnSava() {
            let that = this;
            if (that.SectionChief == "") {
                that.$message({
                    message: "驳回时，工段长处理意见为必填项",
                    type: "error"
                });
            } else {
                let url =
                ...
                that.axios
                    .post(url, {
                        checkId: that.multipleSelection.checkId,
                        rebutRemark: "", // 站位长
                        workRebutRemark: that.SectionChief, // 工段长
                        planRebutRemark: "" // 总计划
                    })
                    .then(res => {
        ...
        handleSelectionChange(val) {
            if (val.length > 1) {
                ...
                if (this.multipleSelection != undefined) {
                    ...
                    this.planRebutRemark = this.multipleSelection.planRebutRemark;
                }
            }
        },
        // 时间转换
